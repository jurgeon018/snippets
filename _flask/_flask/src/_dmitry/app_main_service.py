...
from flask import Flask, Blueprint, jsonify, request, current_app as app
...
def certificate_submit(template, domain, csr):
    submit_url = urllib.parse.urljoin(config.main('windows_service', 'base_url'), "certificate")
    result = requests.post(url=submit_url, json={"domain": domain, "request": csr, "template": template})
    return result.json()
...
def certificate_revoke(certificate_id):
    try:
        certificate_revoke(certificate_id)
...
def certificate_hold(certificate_id):
    try:
        certificate_hold(certificate_id)
...
def certificate_approve_csr(certificate_id):
    ...
    try:
        ...
        result = certificate_submit('', certificate_data[0]['csr'])
        if "result" not in result:
            raise ValueError("incorrect result after certificate submit")
        cer_result = ''.join(result['result'])
